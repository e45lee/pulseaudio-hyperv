...
void pa_resampler_set_input_rate(pa_resampler *r, uint32_t rate) {
    ...
    r->in_frames = pa_resampler_get_delay(r);
    r->out_frames = 0;
 ...
 * call. This is used to preload a resampler after a reset. */
void pa_resampler_prepare(pa_resampler *r, pa_memblockq *history_queue, size_t amount) {
    size_t history_bytes, max_block_size;
    int64_t to_run;
...
void pa_resampler_rewind(pa_resampler *r, size_t out_bytes, pa_memblockq *history_queue, size_t amount) {
    pa_assert(r);
    ...
    if (history_queue && amount > 0)
        pa_resampler_prepare(r, history_queue, amount);
}
...
/* Get delay in input frames */
double pa_resampler_get_delay(pa_resampler *r) {
    double frames;

    frames = r->out_frames * r->i_ss.rate / r->o_ss.rate;
    if (frames >= r->in_frames)
        return 0;
...
pa_usec_t pa_resampler_get_delay_usec(pa_resampler *r) {
    ...
    return (pa_usec_t) (pa_resampler_get_delay(r) * PA_USEC_PER_SEC / r->i_ss.rate);
}
