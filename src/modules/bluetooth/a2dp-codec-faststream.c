...
static const char *choose_remote_endpoint(const pa_hashmap *capabilities_hashmap, const pa_sample_spec *default_sample_spec, bool for_encoding) {
    ...
    PA_HASHMAP_FOREACH_KV(key, a2dp_capabilities, capabilities_hashmap, state) {
        if (!can_accept_capabilities(a2dp_capabilities->buffer, a2dp_capabilities->size, for_encoding))
            continue;
        if (!can_accept_capabilities_mic(a2dp_capabilities->buffer, a2dp_capabilities->size, for_encoding))
            return key;
        if (!key_with_mic)
            key_with_mic = key;
    }
...
static const char *choose_remote_endpoint_mic(const pa_hashmap *capabilities_hashmap, const pa_sample_spec *default_sample_spec, bool for_encoding) {
    const pa_a2dp_codec_capabilities *a2dp_capabilities;
    const char *key;
    void *state;
...
const pa_a2dp_codec pa_a2dp_codec_faststream_mic = {
    ...
    .choose_remote_endpoint = choose_remote_endpoint_mic,
