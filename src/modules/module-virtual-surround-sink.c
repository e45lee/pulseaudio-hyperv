...
PA_MODULE_DESCRIPTION(_("Virtual Surround for Headphones"));
...
PA_MODULE_USAGE(
        _("sink_name=<name for the sink> "
          ...
          "hrir_left=<name of hrir for left output> "
          "hrir_right=<name of optional hrir for right output> "
...
static int sink_input_pop_cb(pa_sink_input *i, size_t nbytes_input, pa_memchunk *chunk) {
    ...
    chunk->index += sink_input_bytes(u->drop);
    chunk->length -= sink_input_bytes(u->drop);
    u->drop = 0;
...
static void sink_input_process_rewind_cb(pa_sink_input *i, size_t nbytes_input) {
    struct userdata *u;
    size_t amount = 0;
    size_t nbytes_sink, nbytes_sink_rounded;
...
int pa__init(pa_module*m) {
    ...
    size_t hrir_right_total_length;
    int hrir_channels;
    int fftlen;
    ...
    pa_proplist_sets(sink_input_data.proplist, PA_PROP_MEDIA_NAME, "Virtual Surround Stream");
    ...
    fftlen = (hrir_samples * 11) / 8; /* Grow a bit for overlap */
    {
